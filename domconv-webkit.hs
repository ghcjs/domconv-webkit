...
declGuard' (Just "DOMNamedFlowCollection") "item" = "WEBKIT_CHECK_VERSION(2,2,2)"
declGuard' (Just "DOMNamedFlowCollection") "namedItem" = "WEBKIT_CHECK_VERSION(2,2,2)"
declGuard' (Just "DOMNamedFlowCollection") "getLength" = "WEBKIT_CHECK_VERSION(2,2,2)"
    ...
    eventimpl iid iat =
      let defget = iid ++ "|" ++ iat ++ "|" ++ eventf intf iat
          ...
          rhs = H.HsUnGuardedRhs $
                  H.HsApp (mkVar "EventName") (H.HsLit (H.HsString (eventName iat)))
...
intf2meth enums intf@(I.Interface _ _ cldefs _ _) =
  concatMap mkmeth (collectOps intf) ++
    ...
    tsig op@(I.Operation (I.FunId _ _ parm) optype _ _ ext) =
      let monadtv = mkTIdent "m"
          -- exprtv = mkTIdent "Expression"
          defop = getDef intf ++ "|" ++ rawName op ++ "|" ++ name op parm
          parms =  mkTIdent "self" : map (fst . tyParm enums) parm
          contxt = concatMap (snd . tyParm enums) parm ++
                     ctxString (optype : map paramType parm)
          -- monadctx = (mkUIdent "Monad",[monadtv])
          thisctx = (mkUIdent (classFor $ getDef intf),[mkTIdent "self"])
          tpsig = mkTsig parms (H.HsTyApp monadtv (tyRet enums optype ext))
          retts = H.HsQualType ((mkUIdent "MonadIO", [mkTIdent "m"]) : thisctx : contxt) tpsig in
      H.HsTypeSig nullLoc [H.HsIdent defop] retts
    timpl op@(I.Operation (I.FunId _ _ parm) optype raises _ ext) =
      let defop = getDef intf ++ "|" ++ rawName op ++ "|" ++ name op parm
          ...
          canRaise = not (null raises) ||
                       I.ExtAttr (I.Id "RaisesException") [] `elem` ext
          ...
          rhs _ _ = returnType enums optype ext rhs'
...
mkEIdent name@(n:_) | n `elem` ("T"::String) = (H.HsEThingAll . H.UnQual . H.HsIdent) name
                    | otherwise = (H.HsEVar . H.UnQual . H.HsIdent) name


-- Obtain a return type signature from a return type

tyRet :: [String] -> I.Type -> [I.ExtAttribute] -> H.HsType

tyRet enums (I.TyOptional t@(I.TyName c Nothing)) ext | isNothing (asIs enums c ext) = tyRet enums t ext
tyRet enums (I.TyOptional t) ext = H.HsTyApp (mkTIdent "Maybe") (tyRet enums t ext)
tyRet enums (I.TySafeArray t) ext = mkTyList (tyRet enums t ext)
tyRet enums (I.TySequence t _) ext = mkTyList (tyRet enums t ext)
tyRet enums (I.TyName c Nothing) ext = case asIs enums c ext of
                                           Nothing -> H.HsTyApp (mkTIdent "Maybe") (mkTIdent $ typeFor c)
                                           Just c' -> mkTIdent c'
tyRet _ I.TyVoid _ = H.HsTyTuple []
tyRet _ (I.TyInteger LongLong) _ = mkTIdent "Int64"
tyRet _ (I.TyInteger _) _ = mkTIdent "Int"
tyRet _ (I.TyFloat Short) _ = mkTIdent "Float"
tyRet _ (I.TyFloat _) _ = mkTIdent "Double"
tyRet _ (I.TyApply (I.TySigned False) (I.TyInteger LongLong)) _ = mkTIdent "Word64"
tyRet _ (I.TyApply (I.TySigned False) (I.TyInteger _)) _ = mkTIdent "Word"
tyRet _ (I.TyApply _ (I.TyInteger LongLong)) _ = mkTIdent "Int64"
tyRet _ (I.TyApply _ (I.TyInteger _)) _ = mkTIdent "Int"
tyRet _ (I.TyObject) _ = mkTIdent "GObject"
tyRet _ (I.TyAny) _ = mkTIdent "(Ptr a)"
tyRet _ t _ = error $ "Return type " ++ show t
...
paramType (I.Param _ _ ptype _ _) = ptype

-- Obtain a type signature from a parameter definition

tyParm :: [String] -> I.Param -> (H.HsType, [H.HsAsst])

tyParm enums (I.Param opt (I.Id p) ptype [I.Mode In] ext) = lookup ptype where
  lookup ptype =
   case ptype of
    I.TyOptional t ->
        case lookup t of
    ...
    I.TyName c Nothing -> case asIs enums c ext of
      Just cc ->  (mkTIdent cc, [])
...
tyParm _ param@(I.Param{}) = error $ "Unsupported parameter attributes " ++ show param

-- Some types pass through as is, other are class names

asIs :: [String] -> String -> [I.ExtAttribute] -> Maybe String

asIs enums a _ | a `elem` enums = Just a
asIs _ "DOMString" ext | I.ExtAttr (I.Id "TreatReturnedNullStringAs") []  `elem` ext = Just "(Maybe string)"
                       | I.ExtAttr (I.Id "TreatNullAs") []  `elem` ext               = Just "(Maybe string)"
                       | otherwise                                                   = Just "string"
asIs _ "DOMString..." _         = Just "string"
asIs _ "DOMTimeStamp" _         = Just "Word"
asIs _ "CompareHow" _           = Just "Word"
asIs _ "Bool" _                 = Just "Bool"
asIs _ "Int" _                  = Just "Int"
asIs _ _ _                      = Nothing
...
applyParam enums param@(I.Param _ (I.Id p) ptype [I.Mode In] ext) call = lookup ptype where
  pname = mkVar $ paramName param
  lookup ptype =
   case ptype of
    I.TyOptional t@(I.TyName c Nothing) | isNothing (asIs enums c ext) -> lookup t
    I.TySequence t _ ->
        H.HsInfixApp
          (H.HsApp
            (mkVar "TODO_seqparm")
    ...
    I.TyName "DOMString" Nothing | I.ExtAttr (I.Id "TreatNullAs") []  `elem` ext ->
        H.HsApp (H.HsApp (H.HsApp (H.HsApp (mkVar "maybeWith") (mkVar "withUTFString")) pname) (mkVar "$"))
           (H.HsLambda (H.SrcLoc "" 1 1) [H.HsPVar . H.HsIdent $ p ++ "Ptr"]
                           $ H.HsApp call (mkVar $ p ++ "Ptr"))
                                 | otherwise ->
        H.HsApp (H.HsApp (H.HsApp (mkVar "withUTFString") pname) (mkVar "$"))
           (H.HsLambda (H.SrcLoc "" 1 1) [H.HsPVar . H.HsIdent $ p ++ "Ptr"]
                           $ H.HsApp call (mkVar $ p ++ "Ptr"))
...
applyParam enums param@(I.Param{}) _ = error $ "Unsupported parameter attributes " ++ show param

returnType :: [String] -> I.Type -> [I.ExtAttribute] -> H.HsExp -> H.HsExp
returnType _ (I.TyName "DOMString" Nothing) ext e
    | I.ExtAttr (I.Id "TreatReturnedNullStringAs") []  `elem` ext
      || I.ExtAttr (I.Id "TreatNullAs") []  `elem` ext
        = H.HsApp (H.HsApp (H.HsParen e) (mkVar ">>=")) (H.HsApp (mkVar "maybePeek") (mkVar "readUTFString"))
    | otherwise = H.HsApp (H.HsApp (H.HsParen e) (mkVar ">>=")) (mkVar "readUTFString")
returnType enums (I.TyName t Nothing) ext e | t `elem` enums = H.HsApp (H.HsApp (mkVar "stringToEnum") (mkVar "<$>")) (H.HsParen (H.HsApp (H.HsApp (H.HsParen e) (mkVar ">>=")) (mkVar "readUTFString")))
returnType _ (I.TyName "DOMTimeStamp" Nothing) _ e = H.HsApp (H.HsApp (mkVar "fromIntegral") (mkVar "<$>")) (H.HsParen e)
returnType _ (I.TyName "CompareHow" Nothing) _ e = H.HsApp (H.HsApp (mkVar "fromIntegral") (mkVar "<$>")) (H.HsParen e)
returnType _ (I.TyName "Bool" Nothing) _ e = H.HsApp (H.HsApp (mkVar "toBool") (mkVar "<$>")) (H.HsParen e)
returnType _ (I.TyName x Nothing) _ e =
  H.HsApp
    ...
    (H.HsParen e)
returnType _ (I.TyInteger _) _ e = H.HsApp (H.HsApp (mkVar "fromIntegral") (mkVar "<$>")) (H.HsParen e)
returnType _ (I.TyFloat _) _ e = H.HsApp (H.HsApp (mkVar "realToFrac") (mkVar "<$>")) (H.HsParen e)
returnType _ (I.TyApply _ (I.TyInteger _)) _ e = H.HsApp (H.HsApp (mkVar "fromIntegral") (mkVar "<$>")) (H.HsParen e)
returnType _ t _ e = e
...
gtkName "EventTarget" "addEventListener" = "event_target_add_event_listener_closure"
gtkName "EventTarget" "removeEventListener" = "event_target_remove_event_listener_closure"
gtkName i s =
